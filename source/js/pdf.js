...
function undiacritic(str) {
  if (typeof str === 'string') {
    ...
    var str_a = [];
    for (s in str) {
      str_a.push(str[s].replace(/[^\u0000-\u007e]/g, function(c) {
        return diacriticsMap[c] || c;
      }));
    }
    return str_a;
  ...
  $(".download-button").click(function() {
    ...
    titles.each(function( index ) {
      ...
      if ($(this).hasClass('break') && first) {
        ...
        // time and header for parallel session (col 1)
        if ($(this).hasClass('parallel') && column == 1) {
          doc.text($(times[i]).text().trim(), 10, y);
          y += 10;
          doc.text(undiacritic($(this).text()), 10, y);
          column = 2;
          return;
        // header for parallel session (col 2)
      } else if ($(this).hasClass('parallel') && column == 2) {
        doc.text(undiacritic($(this).text()), 100, y);

        // individual items within parallel sessions
      ...
      // non parallel sessions (single column)
    } else {
        // time and header
        doc.text($(times[i]).text().trim(), 10, y);
        doc.text(undiacritic($(this).text()), 60, y);

        // detail items if present
